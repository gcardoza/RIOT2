...
//  Revision: January 20th, 2017 - Re-added code to support DHT22 if BME280 is not present
//
...
// ***** Include header files *****
  ...
  #include <Adafruit_BME280.h>      // AdaFruit Library for BME280 Temp, Humidity & Pressure
...
// ***** Set BME280 Temperature, Humidity and Pressure Variables *****
  ...
  int BME280_Present = 1;         // Set default that the BME280 sensor is present
...
void setup(void)
{
  ...
  {
    Serial.println("  -> BME280: Found and initialized sensor.");
    BME280_Present = 1;
  }

  if(!BME280_Present)   //if BME280 is not present assume DHT22 exists
  {
    // ***** Initialize DHT22 Temperature and Humidity sensor *****
    Serial.print("-> DHT22: Starting Temperature & Humidity Sensor\n");
    dht.begin();
    Pressure = 999.9;  //Set pressure to 999.9 as there is no pressure sensor on DHT22
  }
...
void loop()
{
  ...
  if (Current_Time >= Last_Publish_Time + Update_Interval)
  {
    ...
    if(BME280_Present) read_BME280();   // Read BME280 if present
    else read_DHT22();                  // Else assume DHT22 is present
