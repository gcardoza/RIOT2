...
//  Revision: October 31st, 2016 - Normalized SensorData message, added Temp offset, and added MQTT security
//
...
void loop()
{
  ...
  // Support boundary condition if Current_Time counter rolls over 32 bit int
  if(Current_Time < Last_Publish_Time) Last_Publish_Time = Current_Time;
...
{  
  ...
  if(length >=3)
  {
    // The Control Command is the first 3 characters of the Message e.g. UF:, UN:, RS:, IC:, ON:, OF:
  ...
  // ** Default - Unknown command received for the RIOT2  **
